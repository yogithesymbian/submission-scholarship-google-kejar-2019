...
class MoviesTvWapiHomeFragment : androidx.fragment.app.Fragment() {
    ...
    private fun movieTvShowHandle() {
        ...
        if (adapter.itemCount == 0) {
            debuggingMyScode(
                TAG_LOG,
                "adapter tv_show fragment count is 0 , try request api [arrayList.TV_SHOW]"
            )
            ...
            debuggingMyScode(
                TAG_LOG,
                "adapter tv_show fragment is already have item , didn't try request api [arrayList.TV_SHOW]"
            )
            frame_progress_tv_show.visibility = View.GONE
            card_tv_show.visibility = View.VISIBLE
        }
